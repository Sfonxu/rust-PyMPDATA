...
data = (np.full((nx + 1, ny), Cx), np.full((nx, ny + 1), Cy))"#, nx_ny[0], nx_ny[1], Cx_Cy[0], Cx_Cy[1]), None, Some(&full)).unwrap();
        ...
        //state 0 gets pulled as endstate for some unknown reason
        let state_0 = PyDict::new_bound(py);
        //it seems pyo3 has a lot of trouble calling functions if they come from two different imports, so I have to include another code snippet, until workaround is found
        Python::run_bound(py, &"from PyMPDATA import Solver
state_0 = solver.advectee.get().copy()",Some(&[("solver", solver.clone())].into_py_dict_bound(py)), Some(&state_0)).unwrap();
        ...
        let buf0 = format!("{}", state_0);
        let buf = format!("{}", state);
        let mut file0 = File::create("state0.txt")?;
        let mut file = File::create("state.txt")?;
        file0.write_all(buf0.as_bytes())?;
        file.write_all(buf.as_bytes())?;
        Ok(())
    })
    //we can also just bruteforce and run the whole python code without getting rust involved
/*    let run = shortcut::example()?;
    Ok(())*/
}